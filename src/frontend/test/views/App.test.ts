...
const routerTest = async (test: any) => {
  setTimeout(() => {
    test();
  }, 200);
};
...
it('renders the correct navigation links', async () => {
    ...
    routerTest(() => {
      ...
      expect(routerLinks[3].attributes('href')).toBe('https://github.com/RooTender/MTAS');
    });
  });

  it('applies styles to active router link', () => {
    const wrapper = mount(App, {
      global: {
        plugins: [router]
      }
  ...
  it('applies bold font-weight to active router link', () => {
    const wrapper = mount(App, {
      global: {
        plugins: [router]
      }
  ...
  it('underlines router link on hover', () => {
    const wrapper = mount(App, {
    global: {
        plugins: [router]
    }
    ...
    routerTest(() => {
      ...
      expect(activeLink.attributes('style')).toContain('text-decoration: underline;');

      activeLink.trigger('mouseout');
      expect(activeLink.attributes('style')).not.toContain('text-decoration: underline;');
    });
  });
});